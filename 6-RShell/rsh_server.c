...
int process_cli_requests(int svr_socket)
{
    ...
    while (1)
    {
        ...
        if (rc == OK_EXIT || rc == ERR_RDSH_COMMUNICATION)
            break;
    }
...
int exec_client_requests(int cli_socket)
{
    ...
    while (1)
    {
        ...
        {
            send_message_string(cli_socket, "exec:Nosuchfileordirectory");
        }
        else
        {
            char rc_msg[128];
...
int send_message_string(int cli_socket, char *buff)
{
    ...
    {
        bytes = send(cli_socket, buff + total_sent, len - total_sent, 0);
        if (bytes <= 0)
        {
            return ERR_RDSH_COMMUNICATION;
        }
...
int rsh_execute_pipeline(int cli_sock, command_list_t *clist)
{
    ...
    for (int i = 0; i < num_cmds; i++)
    {
        ...
        if (pids[i] == 0)
        {
            ...
            send_message_string(cli_sock, "exec:Nosuchfileordirectory");
    ...
    exit_code = WEXITSTATUS(pids_st[num_cmds - 1]);
    for (int i = 0; i < num_cmds; i++)
    {
